class Api::MoneyController < ApplicationController
  def show
    if params[:type] == "ticker"
      ...
      url = URI.parse("https://public.bitbank.cc/#{params[:id]}_jpy/candlestick/1day/#{this_year}")
      ...
      if params[:candle_type] == "day"
        ...
        url = URI.parse("https://public.bitbank.cc/#{params[:id]}_jpy/candlestick/#{params[:candle_type]}/#{date}")
      end
    end
